...
async def ensure_database_exists(database_url: str, retries: int = 5, base_delay: float = 1.0) -> None:
    """Ensure target database exists. Tries to connect several times with backoff and creates DB if missing."""
    ...
    # asyncpg uses format like: postgresql://user:pass@host:port/db?param=value
...
async def run_alembic_migrations() -> None:
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"Alembic migration error: {result.stderr}")
